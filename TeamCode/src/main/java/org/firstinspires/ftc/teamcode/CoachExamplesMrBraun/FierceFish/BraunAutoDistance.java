...
@Autonomous(name = "BraunAutoDistance")

@Disabled

public class BraunAutoDistance extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            if (frontDistance.getDistance(DistanceUnit.INCH) < 2) {
                // do this, such as LED lights
            }
        }

    }

    void driveForward (double power, double targetDistance, double targetAngle) throws InterruptedException {
        double leftPower;
        double rightPower;
        leftDriveMotor.setPower(power);
        rightDriveMotor.setPower(power);
        while(frontDistance.getDistance(DistanceUnit.INCH) > targetDistance && opModeIsActive()){
